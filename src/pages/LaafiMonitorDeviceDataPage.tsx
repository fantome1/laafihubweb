...
import { HumidityLineChart } from "../components/charts/HumidityLineChart";
...
type State = {
  ...
  humidity: {id: string; value: number; date: string}[];
  exposure: { id: string; value: number; date: string }[];
};
...
class LaafiMonitorDeviceDataPage extends React.PureComponent<Props, State> {
  ...
  componentDidMount(): void {
    ...
    signalRHelper
      .start()
      .then(() => {
        signalRHelper.connection.invoke("SubscribeToGetDeviceData", {
  ...
  listen(data: IReceiveDeviceData) {
    ...
    if (data.dataSent == null) return;

    this.setState((prevState) => {
      return {
        data,
        temperature: [...prevState.temperature, data.dataSent.data.temperature],
        humidity: [...prevState.humidity, data.dataSent.data.humidity],
        exposure: [...prevState.exposure, data.dataSent.data.exposure],
      };
    });
  }
  ...
  goToActivityPage() {
    if (this.state.data)
      this.props.navigate(
        routes.ANOTHER_LAAFI_MONITOR_DEVICE_DATA.build(
          this.state.data.dataSent.data.activityId
        )
      );
  }
  ...
  render() {
    ...
    const position =
      data == null
        ? null
        : {
            lat: data?.dataSent?.data?.coordinates?.latitude,
            lng: data?.dataSent?.data?.coordinates?.longitude,
          };
    ...
    return (
      <div className="bg-[#E5E5E5] px-8 py-2 min-h-[1440px]">
        ...
        <div className="flex space-x-2 mt-12">
          {/* First card */}
          <div
            className="h-[120px] flex justify-between bg-white px-4 rounded-md"
            style={{ flex: "1 1 0" }}
          >
            <div className="relative flex flex-col justify-between">
              <Paper
                ...
                className="absolute top-[-24px] flex justify-center items-center w-[80px] h-[80px]"
              >
                <span className="material-symbols-rounded text-[42px] text-white">
                  devices
                </span>
              </Paper>
              ...
              <div className="flex items-center h-[56px]">
                <div className="flex items-center h-full">
                  <div className="w-[80px] mr-4"></div>{" "}
                  {/* ml marge de l'icone a gauche, 68px (largeur) taille de l'icone */}
              ...
              <div className="mb-2 flex items-center space-x-4">
                ...
                <div>
                  {data == null || !data.dataSent?.bluetoothConnected ? (
                    <span className="material-symbols-rounded text-base">
                      bluetooth
                    </span>
                  ) : (
                    <span className="material-symbols-outlined text-base text-[#309E3A]">
                      bluetooth_connected
                    </span>
                  )}
                </div>
            ...
            <div className="flex flex-col justify-between items-end py-2">
              ...
              {data && (
                <ConnectivityCard
                  data={{
                    deviceType: data.dataSent.network.deviceType,
                    connectionType: data.dataSent.network.connectionType,
                    batteryPercent: data.dataSent.network.phoneBattery,
                  }}
                />
              )}
            </div>
          ...
          {/* Second card */}
          <div
            className="h-[120px] flex flex-col bg-white px-4 rounded-md"
            style={{ flex: "1 1 0" }}
          >
            ...
            <div className="grow flex flex-col justify-around mt-2 mb-2">
              <div className="flex">
                <div className="w-[50%]">
                  <p>FW rev: {data?.dataSent.data.deviceInfo.firmware}</p>
                </div>
                <div className="w-[50%]">
                  <p>SW rev: {data?.dataSent.data.deviceInfo.software}</p>
                </div>
              </div>
              ...
              <div className="flex">
                <div className="w-[50%]">
                  <p>
                    Board rev: {data?.dataSent.data.deviceInfo.hardRevision}
                  </p>
                </div>
                <div className="w-[50%]">
                  <p>Last Update:</p>
        ...
        <div className="flex space-x-4 mt-4 h-[300px]">
            ...
            >
              ...
              >
                <TemperaturePieChart
                  ...
                  thresMinTemp={
                    data?.dataSent?.data?.characteristics?.thresMinTemp
                  }
                  thresMaxTemp={
                    data?.dataSent?.data?.characteristics?.thresMaxTemp
                  }
                />
                ...
                >
                  <p className="text-5xl text-[#3C4858]">
                    {data?.dataSent?.data?.temperature?.value ?? ""}
                  </p>
                  <p className="text-3xl text-[#3C4858]">° C</p>
                  <div className="flex space-x-2">
                    <img src="/icons/laafi_monitor/humidity.svg" alt="" />
                    <p className="text-lg text-[#3C4858]">
                      {data?.dataSent?.data?.humidity?.value
                        ?.toString()
                        ?.padStart(2, "0")}
                      % RH
                    </p>
                ...
                <div className="absolute bottom-[-16px] w-full flex justify-center">
                  <div className="flex justify-between w-[70%]">
                    <p className="text-lg font-medium text-[#3C4858]">
                      {MIN_TEMPERATURE}
                    </p>
                    <p className="text-lg font-medium text-[#3C4858]">
                      {MAX_TEMPERATURE}
          ...
          <div>
            <Paper sx={{ width: "164px", height: "100%" }}>
              <div className="flex flex-col items-center">
                ...
                <div className="flex items-center">
                  <div className="my-4">
                    <BatteryIcon
                      percent={
                        (data?.dataSent?.data?.battery?.value ?? 0) / 100
                      }
                      isCharging={false}
                    />
                  </div>
                  <p className="ml-2 text-2xl text-[#3C4858] font-medium">
                    {(data?.dataSent?.data?.battery?.value ?? 0)
                      .toString()
                      .padStart(2, "0")}
                    %
                  </p>
                </div>
                <p className="text-sm text-[#999999] pb-2">
                  18 heures restantes
                </p>
              </div>
              ...
              <div className="mt-4">
                <div
                  className="w-[90%] border-y border-[#AAAAAA] py-2"
                  style={{ margin: "0 auto" }}
                >
                  <img src="/icons/laafi_monitor/opening_status.svg" alt="" />
                </div>
                <p className="text-lg text-[#3C4858] text-center">
                  Opening status
                </p>
              </div>
            </Paper>
            ...
            {position == null ? (
              ...
              <NearMap key="position" center={position}>
                <Marker position={position}>
                  <Popup>
                    Accuracy {data?.dataSent?.data?.coordinates?.accuracy}
                  </Popup>
                </Marker>
              </NearMap>
            )}
        ...
        <div className="mt-4">
          ...
          >
              ...
              <div>
                <Box>
                  <Tabs
                    value={chartTabIndex}
                    onChange={(_, index) =>
                      this.setState({ chartTabIndex: index })
                    }
                  >
            ...
            <div className="flex flex-col space-y-4 mt-4">
              <Paper
                sx={{ width: "100%", /*height: '300px',*/ padding: "20px" }}
                elevation={0} >
                {chartTabIndex === 0 &&
                  (
                ...
                {chartTabIndex === 1 &&
                  (
                    <HumidityLineChart values={this.state.humidity ?? []} />
                  )}
                {/* <TemperatureLineChart
              ...
              <Paper
                ...
                elevation={0}
              >
                <ExposureChart values={this.state.exposure ?? []} />
              </Paper>
            </div>
          </Paper>
        </div>
...
function ConnectivityCard({
  ...
  switch (data.deviceType) {
    ...
    case "central":
      deviceIcon = (
        <span className="material-symbols-outlined">nest_wifi_router</span>
      );
      break;
  }
